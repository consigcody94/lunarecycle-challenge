...
class WasteStream:
    ...
    NASA Reference Data:
    - ~2 kg/astronaut/day of non-metabolic waste
    - 4-person crew generates 2,100-2,600 kg/year (5.75-7.1 kg/day)
    - Food packaging is ~21% of solid waste (largest category)
    - Packing materials account for ~30% of cargo volume
    """

    # === FOOD PACKAGING (Category 1) - ~21% of waste ===
    # Target: ~1.3-1.5 kg/day for 4 crew
    FOOD_PACKAGING_FILM = WasteItem(
        ...
        mass_per_unit_kg=0.025,  # ~25g per pouch
        volume_per_unit_m3=0.0002,
        density_kg_m3=125.0,
        composition=MaterialComposition(
        ...
        generation_rate_per_crew_day=6.0,  # 6 food items per meal x 3 meals / 3
    ...
    RETORT_POUCH = WasteItem(
        ...
        mass_per_unit_kg=0.035,  # Heavier multi-layer
        volume_per_unit_m3=0.0002,
        density_kg_m3=175.0,
        composition=MaterialComposition(
        ...
        generation_rate_per_crew_day=4.0,  # Main meals
    ...
    BEVERAGE_CONTAINER = WasteItem(
        ...
        generation_rate_per_crew_day=5.0,  # Multiple drinks per day
    ...
    # === CLOTHING (Category 2) - ~15% of waste ===
    # Target: ~0.9-1.0 kg/day for 4 crew
    # Astronauts change clothes every 3-4 days, no laundry
    COTTON_CLOTHING = WasteItem(
        name="Cotton Clothing",
        category=WasteCategory.CLOTHING,
        difficulty=DifficultyLevel.LEVEL_2,
        mass_per_unit_kg=0.200,  # T-shirt ~150g, underwear ~50g
        ...
        generation_rate_per_crew_day=0.75,  # ~1 item every 1.3 days
        decomposition_temp_c=350.0,
    ...
    SYNTHETIC_CLOTHING = WasteItem(
        ...
        category=WasteCategory.CLOTHING,
        difficulty=DifficultyLevel.LEVEL_2,
        mass_per_unit_kg=0.180,  # Socks, athletic wear
        volume_per_unit_m3=0.0015,
        density_kg_m3=120.0,
        composition=MaterialComposition(
        ...
        generation_rate_per_crew_day=0.6,  # Socks changed more frequently
        melting_point_c=260.0,
        decomposition_temp_c=400.0,
    ...
    # === FOAM/PACKING (Category 3) - ~30% of cargo volume ===
    # Target: ~0.6-0.8 kg/day for 4 crew
    ZOTEK_FOAM = WasteItem(
        name="Zotek F30 Foam",
        category=WasteCategory.FOAM_PACKING,
        difficulty=DifficultyLevel.LEVEL_3,  # Hardest to recycle (PFAS concerns)
        mass_per_unit_kg=0.080,
        volume_per_unit_m3=0.003,  # Large volume, low density
        density_kg_m3=27.0,
        composition=MaterialComposition(
            primary_material="PVDF",  # Polyvinylidene fluoride
            primary_percentage=100.0,
        ),
        generation_rate_per_crew_day=1.0,  # From cargo unpacking
    ...
    PE_FOAM = WasteItem(
        ...
        density_kg_m3=25.0,
        composition=MaterialComposition(
            primary_material="PE", primary_percentage=98.0, secondary_materials={"Additives": 2.0}
        ),
        generation_rate_per_crew_day=1.5,  # More common foam type
    ...
    # === HYGIENE ITEMS (Category 4) - ~20% of waste ===
    # Target: ~1.2-1.4 kg/day for 4 crew
    CLEANING_WIPES = WasteItem(
        name="Cleaning Wipes",
        category=WasteCategory.HYGIENE_ITEMS,
        difficulty=DifficultyLevel.LEVEL_1,
        mass_per_unit_kg=0.015,  # Wet wipe ~15g
        ...
        generation_rate_per_crew_day=12.0,  # Personal hygiene, cleaning
        decomposition_temp_c=350.0,
        heating_value_mj_kg=20.0,
        moisture_content_percent=5.0,  # Dry after use
    ...
    TOWELS = WasteItem(
        ...
        category=WasteCategory.HYGIENE_ITEMS,
        difficulty=DifficultyLevel.LEVEL_1,
        mass_per_unit_kg=0.120,  # Hand/face towel
        ...
        generation_rate_per_crew_day=1.5,  # Towels replaced frequently
        decomposition_temp_c=350.0,
        heating_value_mj_kg=17.0,
        commercial_equivalent="Cotton-blend towel",
    )

    # === SCIENCE MATERIALS (Category 5) - ~5% of waste ===
    # Target: ~0.3-0.4 kg/day for 4 crew
    SAMPLE_CONTAINERS = WasteItem(
        ...
        density_kg_m3=300.0,
        composition=MaterialComposition(
            primary_material="PP", primary_percentage=95.0, secondary_materials={"Silicone": 5.0}
        ),
    ...
    EXPERIMENT_CONSUMABLES = WasteItem(
        ...
        generation_rate_per_crew_day=0.6,
        decomposition_temp_c=400.0,
        heating_value_mj_kg=25.0,
        commercial_equivalent="Lab consumables mix",
    )

    # === GENERAL WASTE (Category 6) - ~9% of waste ===
    # Target: ~0.5-0.6 kg/day for 4 crew
    TAPE = WasteItem(
        name="Tape",
        category=WasteCategory.GENERAL_WASTE,
        difficulty=DifficultyLevel.LEVEL_1,
        mass_per_unit_kg=0.008,
        volume_per_unit_m3=0.00002,
        density_kg_m3=400.0,
        composition=MaterialComposition(
            primary_material="PP", primary_percentage=60.0, secondary_materials={"Adhesive": 40.0}
        ),
        generation_rate_per_crew_day=2.0,  # Lots of tape used
        decomposition_temp_c=350.0,
    ...
    PAPER = WasteItem(
        ...
        category=WasteCategory.GENERAL_WASTE,
        difficulty=DifficultyLevel.LEVEL_1,
        mass_per_unit_kg=0.020,  # Labels, notes, etc.
        volume_per_unit_m3=0.0001,
        density_kg_m3=200.0,
        composition=MaterialComposition(
        ...
        generation_rate_per_crew_day=1.5,
    ...
    FILTERS = WasteItem(
        ...
        category=WasteCategory.GENERAL_WASTE,
        difficulty=DifficultyLevel.LEVEL_2,
        mass_per_unit_kg=0.150,  # HEPA filters are heavy
        ...
        generation_rate_per_crew_day=0.15,  # Changed less frequently
        decomposition_temp_c=450.0,
        heating_value_mj_kg=25.0,
